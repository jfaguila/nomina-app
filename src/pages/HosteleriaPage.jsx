...
import {
  CONVENIOS_PUBLICOS,
  CONVENIOS_FICHA,
  SECTOR_HOSTELERIA as S,
  eur,
} from '../data/conveniosPublicos';
import { schemaHosteleria } from '../data/seoSchema';
import SiteFooter from '../components/SiteFooter';

const esHost = (c) => c.slug.startsWith('hosteleria-');

// Con tabla comparable primero: son las que resuelven "cuanto cobra un camarero en X"
// sin que el usuario tenga que seguir buscando.
const CON_TABLA = CONVENIOS_PUBLICOS.filter(esHost);
const FICHAS = CONVENIOS_FICHA.filter(esHost);
...
export default function HosteleriaPage() {
  useSeo({
    ...
    path: S.path,
    jsonLd: schemaHosteleria(TODOS, S.faq),
  });
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-sans">
      ...
      <main className="max-w-4xl mx-auto px-6 pb-20">
        ...
        {CON_TABLA.length > 0 && (
          <>
            <h2 className="text-2xl font-bold mt-10 mb-3">Provincias con tabla salarial publicada</h2>
            <p className="text-gray-700 dark:text-gray-300 mb-5">
              De estos convenios hemos leído el anexo salarial en su boletín oficial, así que
              publicamos la tabla completa por nivel y el verificador puede comparar tu nómina
              contra ella.
            </p>
        ...
        {FICHAS.length > 0 && (
          <>
            <h2 className="text-2xl font-bold mt-10 mb-3">Provincias con ficha informativa</h2>
            <p className="text-gray-700 dark:text-gray-300 mb-5">
              De estas tenemos el convenio identificado —nombre, ámbito, vigencia y, cuando existe,
              el enlace al boletín— pero no hemos podido verificar su tabla salarial. Cada ficha
              explica exactamente qué falta. No publicamos importes que no podamos respaldar.
            </p>
            <div className="grid gap-4 mb-10">
              {FICHAS.map((c) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">{c.ambito} · {c.vigencia}</p>
                </Link>
              ))}
            </div>
          </>
        ...
        <div className="rounded-2xl border border-blue-200 dark:border-blue-900 bg-blue-50 dark:bg-blue-950/40 p-6 mb-12">
          ...
          <p className="text-gray-700 dark:text-gray-300 mb-4">
            NominIA lee tu nómina, ordena los conceptos y te enseña el desglose.
        ...
        <h2 className="text-2xl font-bold mb-4">Preguntas frecuentes sobre el convenio de hostelería</h2>
